...
import { GetSpeakersParams } from "../model/RequestParams";
import { GetSpeakersResponse } from "../model/Speaker";

export const speakersDataSource = {
	...
	// },
	async getSpeakers(
		params?: GetSpeakersParams,
	): Promise<GetSpeakersResponse | undefined> {
		try {
			const res = await apiClient.get<GetSpeakersResponse>("/speaker", {
				params,
			});
			return res.data ?? undefined;
		} catch (error: any) {
			console.error("getSpeakers error:", error.message ?? error);
